...
def test_underspecifiedSingleVis(test_showMore):
	...
	for attr in df.currentView[0].specLst: assert attr.dataModel=="measure"
	for attr in df.currentView[0].specLst: assert attr.dataType=="quantitative"
...
def test_underspecifiedVisCollection(test_showMore):
	...
	assert len(df.currentView) == len([view.getAttrByDataType("quantitative") for view in df.currentView]) # should be 5
	test_showMore(df,multipleViewActions)
	...
	assert len(df.currentView) == len([view.getAttrByDataModel("measure") for view in df.currentView]) #should be 25
	test_showMore(df,multipleViewActions)
...
def test_autoencodingHistogram():
	...
	# assert df.currentView[0].getAttrByChannel("x")[0].attribute == "MilesPerGal"
	# assert df.currentView[0].getAttrByChannel("y")[0].attribute == "count()"
