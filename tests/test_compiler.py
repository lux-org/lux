...
def test_underspecified_no_vis(test_show_more):
	...
	df.set_context([lux.Clause(attribute ="Origin", filter_op="=", value="USA")])
	test_show_more(df, no_view_actions)
	assert len(df.current_context) == 0
...
def test_underspecified_single_vis(test_show_more):
	...
	df = pd.read_csv("lux/data/car.csv")
	df.set_context([lux.Clause(attribute ="MilesPerGal"), lux.Clause(attribute ="Weight")])
...
# 	df.set_context([lux.Clause(attribute = ["Horsepower", "Weight", "Acceleration"]), lux.Clause(attribute ="Year", channel="x")])
...
# 	df.set_context([lux.Clause(attribute ="?"), lux.Clause(attribute ="Year", channel="x")])
# 	assert len(df.current_context) == len(list(df.columns)) - 1 # we remove year by year so its 8 vis instead of 9
# 	for vc in df.current_context:
# 		assert (vc.get_attr_by_channel("x")[0].attribute == "Year")
# 	test_show_more(df, multiple_view_actions)

# 	df.set_context([lux.Clause(attribute ="?", data_type="quantitative"), lux.Clause(attribute ="Year")])
# 	assert len(df.current_context) == len([view.get_attr_by_data_type("quantitative") for view in df.current_context]) # should be 5
# 	test_show_more(df, multiple_view_actions)

# 	df.set_context([lux.Clause(attribute ="?", data_model="measure"), lux.Clause(attribute="MilesPerGal", channel="y")])
# 	for vc in df.current_context:
# 		print (vc.get_attr_by_channel("y")[0].attribute == "MilesPerGal")
# 	test_show_more(df, multiple_view_actions)

# 	df.set_context([lux.Clause(attribute ="?", data_model="measure"), lux.Clause(attribute ="?", data_model="measure")])
...
def test_parse():
	df = pd.read_csv("lux/data/car.csv")
	df.set_context([lux.Clause("Origin=?"), lux.Clause(attribute ="MilesPerGal")])
	assert len(df.current_context) == 3

	df = pd.read_csv("lux/data/car.csv")
	df.set_context([lux.Clause("Origin=?"), lux.Clause("MilesPerGal")])
	assert len(df.current_context) == 3
def test_underspecified_vis_collection_zval():
	...
	# df.set_context([lux.Clause(attribute = ["Origin","Cylinders"], filter_op="=",value="?"),lux.Clause(attribute = ["Horsepower"]),lux.Clause(attribute = "Weight")])
...
def test_autoencoding_scatter():
	...
	df.set_context([lux.Clause(attribute="MilesPerGal", channel="y"), lux.Clause(attribute="Weight")])
	view = df.current_context[0]
	check_attribute_on_channel(view, "MilesPerGal", "y")
	check_attribute_on_channel(view, "Weight", "x")

	# Full channel specified
	df.set_context([lux.Clause(attribute="MilesPerGal", channel="y"), lux.Clause(attribute="Weight", channel="x")])
	view = df.current_context[0]
	check_attribute_on_channel(view, "MilesPerGal", "y")
	check_attribute_on_channel(view, "Weight", "x")
	...
	with pytest.raises(ValueError):
		...
		df.set_context([lux.Clause(attribute="MilesPerGal", channel="x"), lux.Clause(attribute="Weight", channel="x")])
...
def test_autoencoding_histogram():
	...
	df = pd.read_csv("lux/data/cars.csv")
	df["Year"] = pd.to_datetime(df["Year"], format='%Y')  # change pandas dtype for the column "Year" to datetype
	df.set_context([lux.Clause(attribute="MilesPerGal", channel="y")])
	view = df.current_context[0]
	check_attribute_on_channel(view, "MilesPerGal", "y")

	# Record instead of count
	# df.set_context([lux.Clause(attribute="MilesPerGal",channel="x")])
...
def test_autoencoding_line_chart():
	df = pd.read_csv("lux/data/cars.csv")
	df["Year"] = pd.to_datetime(df["Year"], format='%Y')  # change pandas dtype for the column "Year" to datetype
	df.set_context([lux.Clause(attribute="Year"), lux.Clause(attribute="Acceleration")])
	view = df.current_context[0]
	check_attribute_on_channel(view, "Year", "x")
	check_attribute_on_channel(view, "Acceleration", "y")

	# Partial channel specified
	df.set_context([lux.Clause(attribute="Year", channel="y"), lux.Clause(attribute="Acceleration")])
	view = df.current_context[0]
	check_attribute_on_channel(view, "Year", "y")
	check_attribute_on_channel(view, "Acceleration", "x")

	# Full channel specified
	df.set_context([lux.Clause(attribute="Year", channel="y"), lux.Clause(attribute="Acceleration", channel="x")])
	view = df.current_context[0]
	check_attribute_on_channel(view, "Year", "y")
	check_attribute_on_channel(view, "Acceleration", "x")
	...
	with pytest.raises(ValueError):
		# Should throw error because there should not be columns with the same channel specified
		df.set_context([lux.Clause(attribute="Year", channel="x"), lux.Clause(attribute="Acceleration", channel="x")])

def test_autoencoding_color_line_chart():
	df = pd.read_csv("lux/data/cars.csv")
	df["Year"] = pd.to_datetime(df["Year"], format='%Y')  # change pandas dtype for the column "Year" to datetype
	df.set_context([lux.Clause(attribute="Year"), lux.Clause(attribute="Acceleration"), lux.Clause(attribute="Origin")])
...
def test_autoencoding_color_scatter_chart():
	df = pd.read_csv("lux/data/cars.csv")
	df["Year"] = pd.to_datetime(df["Year"], format='%Y')  # change pandas dtype for the column "Year" to datetype
	df.set_context([lux.Clause(attribute="Horsepower"), lux.Clause(attribute="Acceleration"), lux.Clause(attribute="Origin")])
	view = df.current_context[0]
	check_attribute_on_channel(view, "Origin", "color")

	df.set_context([lux.Clause(attribute="Horsepower"), lux.Clause(attribute="Acceleration", channel="color"), lux.Clause(attribute="Origin")])
	view = df.current_context[0]
...
def test_populate_options():
	from lux.compiler.Compiler import Compiler
	df = pd.read_csv("lux/data/cars.csv")
	df.set_context([lux.Clause(attribute="?"), lux.Clause(attribute="MilesPerGal")])
	col_set = set()
	...
	df.set_context([lux.Clause(attribute="?", data_model="measure"), lux.Clause(attribute="MilesPerGal")])
	col_set = set()
	for specOptions in Compiler.populate_wildcard_options(df.context, df)["attributes"]:
		for clause in specOptions:
			col_set.add(clause.attribute)
