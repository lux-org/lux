...
def test_lazy_execution():
    ...
    assert df.current_intent[0].data == None
    PandasExecutor.execute(df.current_intent, df)
    assert type(df.current_intent[0].data) == lux.luxDataFrame.LuxDataframe.LuxDataFrame
...
def test_selection():
    ...
    assert all([type(vc.data) == lux.luxDataFrame.LuxDataframe.LuxDataFrame for vc in df.current_intent])
    assert all(df.current_intent[2].data.columns == ["Year", 'Acceleration'])

def test_aggregation():
    df = pd.read_csv("lux/data/car.csv")
    df.set_intent([lux.Clause(attribute ="Horsepower", aggregation="mean"), lux.Clause(attribute ="Origin")])
    PandasExecutor.execute(df.current_intent, df)
    result_df = df.current_intent[0].data
...
def test_inequalityfilter():
    ...
    # Test end-to-end
    PandasExecutor.execute(df.current_intent, df)
    Nbins =list(filter(lambda x: x.bin_size!=0, df.current_intent[0]._inferred_intent))[0].bin_size
    assert len(df.current_intent[0].data) == Nbins
...
def test_record():
    ...
    PandasExecutor.execute(df.current_intent, df)
    assert len(df.current_intent[0].data) == len(df["Cylinders"].unique())
...
def test_filter_aggregation_fillzero_aligned():
    ...
    PandasExecutor.execute(df.current_intent, df)
    result = df.current_intent[0].data
