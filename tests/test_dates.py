...
def test_period_selection():
	...
	PandasExecutor.execute(ldf.current_intent, ldf)

	assert all([type(vc.data) == lux.luxDataFrame.LuxDataframe.LuxDataFrame for vc in ldf.current_intent])
	assert all(ldf.current_intent[2].data.columns == ["Year", 'Acceleration'])
