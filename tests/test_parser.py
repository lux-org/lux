...
def test_case1():
	...
	assert(type(ldf.context[0]) is lux.VisSpec)
	assert(ldf.context[0].attribute == "Horsepower")

def test_case2():
	ldf = pd.read_csv("lux/data/car.csv")
	ldf.set_context(["Horsepower", lux.VisSpec("MilesPerGal", channel="x")])
	assert(type(ldf.context[0]) is lux.VisSpec)
	assert(ldf.context[0].attribute == "Horsepower")
	assert(type(ldf.context[1]) is lux.VisSpec)
...
lux.set_context(["Horsepower","MPG","Acceleration"],"Origin")
	lux.set_context("Horsepower/MPG/Acceleration", "Origin")
		--> [VisSpec(attr= ["Horsepower","MPG","Acceleration"], type= "attributeGroup")]
'''