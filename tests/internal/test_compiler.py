...
def test_underspecified_no_vis(global_var, test_recs):
    ...
    assert df.lux.current_vis is None or len(df.lux.current_vis) == 0

    # test only one filter context case.
    df.lux.set_intent(
        [lux.Clause(attribute="Origin", filter_op="=", value="USA")])
    test_recs(df, no_vis_actions)
    assert len(df.lux.current_vis) == 0
    df.lux.clear_intent()
...
def test_underspecified_single_vis(global_var, test_recs):
    ...
    df = pytest.car_df
    df.lux.set_intent([lux.Clause(attribute="MilesPerGal"),
                      lux.Clause(attribute="Weight")])
    test_recs(df, one_vis_actions)
    assert len(df.lux.current_vis) == 1
    assert df.lux.current_vis[0].mark == "scatter"
    for attr in df.lux.current_vis[0]._inferred_intent:
...
# 	df.lux.set_intent([lux.Clause(attribute = ["Horsepower", "Weight", "Acceleration"]), lux.Clause(attribute ="Year", channel="x")])
# 	assert len(df.lux.current_vis) == 3
# 	assert df.lux.current_vis[0].mark == "line"
# 	for vlist in df.lux.current_vis:
# 		assert (vlist.get_attr_by_channel("x")[0].attribute == "Year")
# 	test_recs(df, multiple_vis_actions)

# 	df.lux.set_intent([lux.Clause(attribute ="?"), lux.Clause(attribute ="Year", channel="x")])
# 	assert len(df.lux.current_vis) == len(list(df.columns)) - 1 # we remove year by year so its 8 vis instead of 9
# 	for vlist in df.lux.current_vis:
# 		assert (vlist.get_attr_by_channel("x")[0].attribute == "Year")
# 	test_recs(df, multiple_vis_actions)

# 	df.lux.set_intent([lux.Clause(attribute ="?", data_type="quantitative"), lux.Clause(attribute ="Year")])
# 	assert len(df.lux.current_vis) == len([vis.get_attr_by_data_type("quantitative") for vis in df.lux.current_vis]) # should be 5
# 	test_recs(df, multiple_vis_actions)

# 	df.lux.set_intent([lux.Clause(attribute ="?", data_model="measure"), lux.Clause(attribute="MilesPerGal", channel="y")])
# 	for vlist in df.lux.current_vis:
# 		print (vlist.get_attr_by_channel("y")[0].attribute == "MilesPerGal")
# 	test_recs(df, multiple_vis_actions)

# 	df.lux.set_intent([lux.Clause(attribute ="?", data_model="measure"), lux.Clause(attribute ="?", data_model="measure")])
# 	assert len(df.lux.current_vis) == len([vis.get_attr_by_data_model("measure") for vis in df.lux.current_vis]) #should be 25
# 	test_recs(df, multiple_vis_actions)
...
def test_set_intent_as_vis(global_var, test_recs):
    ...
    vis = df.lux.recommendation["Correlation"][0]
    df.lux.intent = vis
    df._ipython_display_()
...
def test_parse(global_var):
    ...
    df = pytest.car_df
    vlst = VisList(
        [lux.Clause("Origin=?"), lux.Clause(attribute="MilesPerGal")], df)
    assert len(vlst) == 3
...
def test_populate_options(global_var):
    ...
    df = pytest.car_df
    df.lux.set_intent([lux.Clause(attribute="?"),
                      lux.Clause(attribute="MilesPerGal")])
    col_set = set()
    for specOptions in Compiler.populate_wildcard_options(df.lux._intent, df)["attributes"]:
