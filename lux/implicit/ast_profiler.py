...
class Analyzer(ast.NodeVisitor):
    ...
    def handle_assign(self, node):
        ...
        for t in node.targets:
            df_name, cols = self.get_df_col(t)
            self.dict_add_extend(all_t, df_name, cols)
        
        # parse value 
        if type(v) == ast.BinOp:
            left_df, left_cols = self.get_df_col(v.left)
            right_df, right_cols = self.get_df_col(v.right)
        ...
        for df_key, col_list in all_t.items():
            if df_key in self.df_meta:
                ...
                for hist_item in call_h:
                    col_list.extend(hist_item.cols)
                    if hist_item.f_name:
        ...
        if type(call_node.func) == ast.Attribute or ( 
            ...
            if type(call_node.func) == ast.Attribute:
                ...
                if type(v) == ast.Call: # for df.groupby().agg     "df_name cols f_name f_arg_dict"
                    ...
                    df_name, cols = self.get_df_col(v)
    ...
    def parse_call_args(self, call_node, df_name = ""):
        ...
        for a in call_node.args:
            if (type(a) == ast.Attribute) or (type(a) == ast.Subscript):
                df_name, cols = self.get_df_col(a)
    ...
    def get_df_col(self, v):
        ...
        if type(v) == ast.Attribute:
            ...
            df_name, cols = self.subs_to_str(v)
    ...
    def subs_to_str(self, subs_node):
        ...
        v_str = self.parse_value(subs_node.value)
        
        # slice will be constant or name if single item
        if type(subs_node.slice) == ast.Compare:
            cd = self.parse_compare(subs_node.slice)
            slice_str = cd[v_str] # NOTE ignoring the case where inner compare is different like df_A[df_B.col > 1]
        else:
            slice_str = self.parse_value(subs_node.slice)
    ...
    def parse_compare(self, comp_node):
        ...
        d = {}
        df, col = self.get_df_col(comp_node.left)
        self.dict_add_extend(d, df, col)

        for c in comp_node.comparators:
            df, col = self.get_df_col(c)
            self.dict_add_extend(d, df, col)
