...
def register_default_actions():
    ...
    def no_vis(ldf): return (ldf.current_vis is None) or (
        ldf.current_vis is not None and len(ldf.current_vis) == 0
    )

    def one_current_vis(ldf): return ldf.current_vis is not None and len(
        ldf.current_vis) == 1

    def multiple_current_vis(
        ldf): return ldf.current_vis is not None and len(ldf.current_vis) > 1
