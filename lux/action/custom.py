...
def custom_actions(ldf):
    ...
    """
    if len(lux.actions) > 0:
        recommendations = []
        for action_name in lux.actions.keys():
            display_condition = lux.actions[action_name].display_condition
            if display_condition is None or (display_condition is not None and display_condition(ldf)):
                args = lux.actions[action_name].args
                if args:
                    recommendation = lux.actions[action_name].action(ldf, args)
                else:
                    recommendation = lux.actions[action_name].action(ldf)
