...
def patch_library(DataFrame: tp.Type[LuxDataFrame], Series: tp.Type[LuxSeries]) -> None:
    ...
    def _constructor(self: LuxDataFrame):
        def _construct_and_copy(*args, **kwargs) -> DataFrame:
            ...
            df._LUX_ = LuxDataFrameMethods.from_lux_object(
                "df", df, self.lux)
            ...
            if self.lux._pandas_only:
                ...
                if not self.index.nlevels >= 2 or self.columns.nlevels >= 2:
                    ...
                    if self.lux._intent != [] and (not hasattr(self.lux, "_compiled") or not self.lux._compiled):
                        ...
                        self.lux.current_vis = Compiler.compile_intent(
    ...
    #     ret_val.lux._prev = self
    #     ret_val.lux._history.append_event("head", n=5)
    #     return ret_val
    ...
    #     ret_val.lux._prev = self
    #     ret_val.lux._history.append_event("tail", n=5)
    #     return ret_val

    @patch(DataFrame)
    def groupby(self: LuxDataFrame, *args, history=None, **kwargs):
        return self._super_groupby(*args, **kwargs)
...
# patch pandas
patch_library(pd.DataFrame, pd.Series)  # type: ignore
