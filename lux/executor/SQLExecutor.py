...
class SQLExecutor(Executor):
    ...
    def execute_preview(tbl: LuxSQLTable, preview_size=5):
        ...
        output = pandas.read_sql(preview_query.format(
            table_name=tbl.table_name, num_rows=preview_size), lux.CONFIG.SQLconnection)
        return output
    ...
    def execute_sampling(tbl: LuxSQLTable):
        ...
        length_query = pandas.read_sql(lux.CONFIG.query_templates['length_query'].format(
            table_name=tbl.table_name, where_clause=""), lux.CONFIG.SQLconnection,)
        limit = int(list(length_query["length"])[0]) * SAMPLE_FRAC
        sample_query = lux.CONFIG.query_templates['sample_query'].format(
            table_name=tbl.table_name, where_clause="", num_rows=str(int(limit)))
    ...
    def execute(view_collection: VisList, tbl: LuxSQLTable, approx: bool = False):
        ...
        for view in view_collection:
            ...
            if view.mark == "scatter":
                ...
                length_query = pandas.read_sql(lux.CONFIG.query_templates['length_query'].format(
                    table_name=tbl.table_name, where_clause=where_clause), lux.CONFIG.SQLconnection,)
    ...
    def execute_scatter(view: Vis, tbl: LuxSQLTable):
        ...
        row_count = list(pandas.read_sql(lux.CONFIG.query_templates['length_query'].format(
            table_name=tbl.table_name, where_clause=where_clause), lux.CONFIG.SQLconnection,)["length"])[0]
        if row_count > lux.CONFIG.sampling_cap:
            query = lux.CONFIG.query_templates['sample_query'].format(
                columns=required_variables_str, table_name=tbl.table_name, where_clause=where_clause, num_rows=10000)
            #query = f"SELECT {required_variables} FROM {tbl.table_name} {where_clause} ORDER BY random() LIMIT 10000"
        else:
            query = lux.CONFIG.query_templates['scatter_query'].format(
                columns=required_variables_str, table_name=tbl.table_name, where_clause=where_clause)
        ...
        tbl._message.add_unique(
            f"Large scatterplots detected: Lux is automatically binning scatterplots to heatmaps.", priority=98,)
    ...
    def execute_aggregate(view: Vis, tbl: LuxSQLTable, isFiltered=True):
        ...
        if measure_attr != "":
            ...
            if measure_attr.attribute == "Record":
                ...
                if has_color:
                    count_query = lux.CONFIG.query_templates['colored_barchart_counts'].format(
                        groupby_attr=groupby_attr.attribute, color_attr=color_attr.attribute, table_name=tbl.table_name, where_clause=where_clause,)
                    view._vis_data = pandas.read_sql(
                        count_query, lux.CONFIG.SQLconnection)
                    assert((len(view._vis_data.columns) == 3) &
                           ("count" in view._vis_data.columns))
                    view._vis_data = view._vis_data.rename(
                ...
                # generates query for normal barchart case
                    ...
                    count_query = lux.CONFIG.query_templates['barchart_counts'].format(
                        groupby_attr=groupby_attr.attribute, table_name=tbl.table_name, where_clause=where_clause,)
                    view._vis_data = pandas.read_sql(
                        count_query, lux.CONFIG.SQLconnection)
                    assert((len(view._vis_data.columns) == 2) &
                           ("count" in view._vis_data.columns))
                    view._vis_data = view._vis_data.rename(
                        columns={"count": "Record"})
                    view._vis_data = utils.pandas_to_lux(view._vis_data)
            ...
            # aggregate barchart case, need aggregate data (mean, sum, max) for each group
                ...
                if has_color:
                    if agg_func == "mean":
                        agg_query = (lux.CONFIG.query_templates['colored_barchart_average'].format(
                            groupby_attr=groupby_attr.attribute, color_attr=color_attr.attribute, measure_attr=measure_attr.attribute, table_name=tbl.table_name, where_clause=where_clause,))
                        view._vis_data = pandas.read_sql(
                            agg_query, lux.CONFIG.SQLconnection)
                        assert((len(view._vis_data.columns) == 3) & (
                            measure_attr.attribute in view._vis_data.columns))
                        view._vis_data = utils.pandas_to_lux(view._vis_data)
                    if agg_func == "sum":
                        agg_query = (lux.CONFIG.query_templates['colored_barchart_sum'].format(
                            groupby_attr=groupby_attr.attribute, color_attr=color_attr.attribute, measure_attr=measure_attr.attribute, table_name=tbl.table_name, where_clause=where_clause,))
                        view._vis_data = pandas.read_sql(
                            agg_query, lux.CONFIG.SQLconnection)
                        assert((len(view._vis_data.columns) == 3) & (
                        ...
                        view._vis_data = utils.pandas_to_lux(view._vis_data)
                    if agg_func == "max":
                        agg_query = (lux.CONFIG.query_templates['colored_barchart_max'].format(
                            groupby_attr=groupby_attr.attribute, color_attr=color_attr.attribute, measure_attr=measure_attr.attribute, table_name=tbl.table_name, where_clause=where_clause,))
                        view._vis_data = pandas.read_sql(
                            agg_query, lux.CONFIG.SQLconnection)
                        assert((len(view._vis_data.columns) == 3) & (
                ...
                # generates query for normal barchart case
                    ...
                    if agg_func == "mean":
                        agg_query = lux.CONFIG.query_templates['barchart_average'].format(
                            groupby_attr=groupby_attr.attribute, measure_attr=measure_attr.attribute, table_name=tbl.table_name, where_clause=where_clause,)
                        view._vis_data = pandas.read_sql(
                            agg_query, lux.CONFIG.SQLconnection)
                        assert((len(view._vis_data.columns) == 2) & (
                            measure_attr.attribute in view._vis_data.columns))
            ...
            if isFiltered or has_color and attr_unique_vals:
                ...
                if len(result_vals) != N_unique_vals * color_cardinality:
                    ...
                    if has_color:
                        df = pandas.DataFrame({columns[0]: attr_unique_vals * color_cardinality,
                        ...
                        for col in columns[1:]:
                            view._vis_data[col] = view._vis_data[col].fillna(0)
                        assert (len(list(view._vis_data[groupby_attr.attribute])) ==
    ...
    def execute_binning(view: Vis, tbl: LuxSQLTable):
        ...
        if "cases" in lux.CONFIG.query_templates['histogram_counts']:
            ...
            for i in range(1, len(bucket_edges)):
                when_lines = when_lines + when_line.format(
                    column=bin_attribute.attribute, lower_edge=bucket_edges[i-1], upper_edge=bucket_edges[i], label=str(i-1)) + " "
            ...
            bin_count_query = lux.CONFIG.query_templates['histogram_counts'].format(
                bucket_cases=when_lines, table_name=tbl.table_name, where_clause=where_clause)
        # need to calculate the bin edges before querying for the relevant data
        else:
            bin_count_query = lux.CONFIG.query_templates['histogram_counts'].format(
                bin_attribute=bin_attribute.attribute, upper_edges="{" + upper_edges + "}", table_name=tbl.table_name, where_clause=where_clause,)

        bin_count_data = pandas.read_sql(
            bin_count_query, lux.CONFIG.SQLconnection)
        assert((len(bin_count_data.columns) == 2) & (
            set(['width_bucket', 'count']).issubset(bin_count_data.columns)))
        if not bin_count_data["width_bucket"].isnull().values.any():
            ...
            bin_centers = np.append(bin_centers, np.mean(
                np.vstack([upper_edges[0:-1], upper_edges[1:]]), axis=0),)
            if attr_type == int:
                bin_centers = np.append(bin_centers, math.ceil(
            ...
            view._vis_data = pandas.DataFrame(np.array([bin_centers, list(
                bin_count_data["count"])]).T, columns=[bin_attribute.attribute, "Number of Records"],)
    ...
    def execute_2D_binning(view: Vis, tbl: LuxSQLTable):
        ...
        x_attribute = list(filter(lambda x: x.channel ==
                           "x", view._inferred_intent))[0]

        y_attribute = list(filter(lambda x: x.channel ==
                           "y", view._inferred_intent))[0]
        ...
        if "cases" in lux.CONFIG.query_templates['histogram_counts']:
            ...
            for i in range(1, len(x_bucket_edges)):
                x_when_lines = x_when_lines + when_line.format(
                    column=x_attribute.attribute, lower_edge=x_bucket_edges[i-1], upper_edge=x_bucket_edges[i], label=str(i-1)) + " "
                y_when_lines = y_when_lines + when_line.format(
                    column=y_attribute.attribute, lower_edge=y_bucket_edges[i-1], upper_edge=y_bucket_edges[i], label=str(i-1)) + " "
            ...
            bin_count_query = lux.CONFIG.query_templates['heatmap_counts'].format(
                bucket_cases1=x_when_lines, bucket_cases2=y_when_lines, table_name=tbl.table_name, where_clause=where_clause)

        else:
            bin_count_query = lux.CONFIG.query_templates['heatmap_counts'].format(
                x_attribute=x_attribute.attribute, x_upper_edges_string="{" + x_upper_edges_string + "}", y_attribute=y_attribute.attribute, y_upper_edges_string="{" + y_upper_edges_string + "}", table_name=tbl.table_name, where_clause=where_clause,)

        # data = pandas.read_sql(bin_count_query, lux.config.SQLconnection)

        data = pandas.read_sql(bin_count_query, lux.CONFIG.SQLconnection)
        assert((len(data.columns) == 3) & (
            set(['width_bucket1', 'width_bucket2', 'count']).issubset(data.columns)))
        # data = data[data["width_bucket1"] != num_bins - 1]
        # data = data[data["width_bucket2"] != num_bins - 1]
        if len(data) > 0:
            data["xBinStart"] = data.apply(lambda row: float(
                x_upper_edges[int(row["width_bucket1"]) - 1]) - x_bin_width, axis=1)
            data["xBinEnd"] = data.apply(lambda row: float(
                x_upper_edges[int(row["width_bucket1"]) - 1]), axis=1)
            data["yBinStart"] = data.apply(lambda row: float(
                y_upper_edges[int(row["width_bucket2"]) - 1]) - y_bin_width, axis=1)
            data["yBinEnd"] = data.apply(lambda row: float(
                y_upper_edges[int(row["width_bucket2"]) - 1]), axis=1)
        view._vis_data = utils.pandas_to_lux(data)
    ...
    def get_filtered_size(filter_specs, tbl):
        clause_info = SQLExecutor.create_where_clause(
            filter_specs=filter_specs, view="")
        where_clause = clause_info[0]
        filter_intents = filter_specs[0]
        filtered_length = pandas.read_sql(lux.CONFIG.query_templates['length_query'].format(
            table_name=tbl.table_name, where_clause=where_clause), lux.CONFIG.SQLconnection,)
    ...
    def get_SQL_attributes(self, tbl: LuxSQLTable):
        ...
        attributes = list(pandas.read_sql(
            attr_query, lux.CONFIG.SQLconnection)["column_name"])
    ...
    def compute_stats(self, tbl: LuxSQLTable):
        ...
        for attribute in tbl.columns:
            if tbl._data_type[attribute] == "quantitative":
                min_max_query = pandas.read_sql(lux.CONFIG.query_templates['min_max_query'].format(
                    attribute=attribute, table_name=tbl.table_name), lux.CONFIG.SQLconnection,)
                tbl._min_max[attribute] = (list(min_max_query["min"])[
                                           0], list(min_max_query["max"])[0],)
    ...
    def get_unique_values(self, tbl: LuxSQLTable):
        ...
        for attr in list(tbl.columns):
            ...
            unique_data = pandas.read_sql(
                unique_query, lux.CONFIG.SQLconnection,)
    ...
    def compute_data_type(self, tbl: LuxSQLTable):
        ...
        for attr in list(tbl.columns):
            ...
            datatype = list(pandas.read_sql(
                datatype_query, lux.CONFIG.SQLconnection)["data_type"])[0]
