...
class SQLExecutor(Executor):
    ...
    def execute_binning(vis:Vis, ldf:LuxDataFrame):
        ...
        if not math.isnan(vis.data.min_max[bin_attribute.attribute][0]) and math.isnan(vis.data.min_max[bin_attribute.attribute][1]):
            ...
            vis._vis_data = pd.DataFrame(np.array([bin_centers,list(bin_count_data['count'])]).T,columns=[bin_attribute.attribute, "Number of Records"])
            vis._vis_data = utils.pandas_to_lux(vis.data)
