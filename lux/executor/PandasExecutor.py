    ...
    def execute_aggregate(vis: Vis, isFiltered=True):
        ...
        if measure_attr != "":
            if measure_attr.attribute == "Record":
                ...
                if has_color:
                    vis._vis_data = (
                        vis.data._groupby([groupby_attr.attribute, color_attr.attribute], dropna=False)
                        .count()
                        .reset_index()
                    ...
                    vis._vis_data = (
                        vis.data._groupby(groupby_attr.attribute, dropna=False)
                        .count()
                        .reset_index()
                        .rename(columns={index_name: "Record"})
                    )
                ...
                # if color is specified, need to group by groupby_attr and color_attr
                if has_color:
                    groupby_result = vis.data._groupby(
                        [groupby_attr.attribute, color_attr.attribute], dropna=False
                    )
                else:
                    groupby_result = vis.data._groupby(groupby_attr.attribute, dropna=False)
