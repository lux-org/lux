...
class LuxDataFrame(pd.DataFrame):
    ...
    def current_vis_to_JSON(vlist, input_current_vis=""):
        ...
        if numVC == 1:
            plot_backend = lux.config.plotting_backend
            if lux.config.plotting_backend == "matplotlib":
                plot_backend = "to_matplotlib_svg_dict"
            current_vis_spec = vlist[0].to_code(language=plot_backend, prettyOutput=False)
        ...
        for idx, rec in enumerate(rec_copy):
            if len(rec["collection"]) > 0:
                ...
                for vis in rec["collection"]:
                    ...
                    if lux.config.plotting_backend == "matplotlib":
                        plot_backend = "to_matplotlib_svg_dict"
                    chart = vis.to_code(language=plot_backend, prettyOutput=False)
