...
class LuxDataFrame(object):
    _metadata: tp.List[str]
    _sampled: tp.Any
    _approx_sample: tp.Any
...
def intent(self, intent_input: Union[List[Union[str, Clause]], Vis]):
    ...
    if is_list_input:
        ...
        set_intent_as_vis(self, intent_input)
...
def copy_intent(df):
    ...
    output = []
    for clause in df._intent:
        temp_clause = clause.copy_clause()
        output.append(temp_clause)
    return output


def set_intent_as_vis(df, vis: Vis):
    """
    ...
    """
    df.expire_recs()
    df._intent = vis._inferred_intent
    df._parse_validate_compile_intent()
...
def current_vis(self):
    ...
    if valid_current_vis and Validator.validate_intent(self._current_vis[0].intent, self):
        config.executor.execute(self._current_vis, self)
    ...
    if lazy_but_not_computed or eager:
        ...
        if rec_df.pre_aggregated:
            if rec_df.columns.name is not None:
                _append_rec(rec_df, rec_infolist, row_group(rec_df))
            _append_rec(rec_df, rec_infolist, column_group(rec_df))
            ...
            for rec in custom_action_collection:
                _append_rec(rec_df, rec_infolist, rec)
...
def remove_deleted_recs(df, change):
    for action in df._widget.deletedIndices:
        deletedSoFar = 0
        for index in df._widget.deletedIndices[action]:
            df._recommendation[action].remove_index(index - deletedSoFar)
            deletedSoFar += 1


def set_intent_on_click(df, change):
    from IPython.display import clear_output, display
    from lux.processor.Compiler import Compiler

    intent_action = list(df._widget.selectedIntentIndex.keys())[0]
    vis = df._recommendation[intent_action][df._widget.selectedIntentIndex[intent_action][0]]
    set_intent_as_vis(df, vis)

    maintain_metadata(df)
    df.current_vis = Compiler.compile_intent(df, df._intent)
    maintain_recs(df)
    ...
    df._widget.observe(lambda change: remove_deleted_recs(
        df, change), names="deletedIndices")
    df._widget.observe(lambda change: set_intent_on_click(
        df, change), names="selectedIntentIndex")
...
def _ipython_display_(self):
    ...
    try:
        if self._pandas_only:
            ...
            # maintain_recs(df_to_display) # compute the recommendations (TODO: This can be rendered in another thread in the background to populate self._widget)
            maintain_recs(self)

            # Observers(callback_function, listen_to_this_variable)
            self._widget.observe(lambda change: remove_deleted_recs(self, change),
                                 names="deletedIndices")
            self._widget.observe(lambda change: set_intent_on_click(self, change),
...
@patch(DataFrame)
def display_pandas(self: DataFrame):
    df = self.copy()
...
@patch(DataFrame)
def render_widget(self: DataFrame, renderer: str = "altair", input_current_vis=""):
    """
    ...
    return luxwidget.LuxWidget(
        ...
        intent=intent_to_string(self._intent),
...
def to_JSON(self, rec_infolist, input_current_vis=""):
    widget_spec = {}
    if self.current_vis:
        config.executor.execute(self.current_vis, self)
