...
.. code-block:: python
  ...
  def to_lux_panel(df, style="width:100%;height:100%", frameborder="0"):
      html_content = df.save_as_html(output=True)
      html_content = html.escape(html_content)
      iframe_report = f"""<iframe srcdoc="{html_content}" style={style} frameborder="{frameborder}"
  "allowfullscreen></iframe>"""

      return pn.pane.HTML(iframe_report, height=425)
